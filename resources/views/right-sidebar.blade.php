<!-- Right Sidebar -->
<div
    class="border-gray-750 absolute right-0 z-100 flex h-screen w-64 flex-shrink-0 select-none flex-col border-l bg-gray-800 text-white">
    <div class="flex h-10 flex-shrink-0 items-center border-b border-gray-800 bg-gray-800">
        <div
            class="flex h-full w-full cursor-pointer items-center justify-center border-b border-transparent bg-gray-800 text-sm text-xs text-gray-100">
            Design</div>

        <div
            class="border-gray-750 flex h-full w-full cursor-pointer items-center justify-center border-b bg-gray-900 text-sm text-xs text-gray-400 hover:text-gray-100">
            History</div>
    </div>

    <div class="relative h-full w-auto">
        <div class="border-gray-750 flex flex-col border-b">
            <div class="p-4 text-center">
                <div
                    class="flex w-full flex-col items-center justify-center rounded-md border-0 border-gray-700 bg-gray-900 py-6 px-5 text-gray-400">
                    <svg class="h-7 w-7 fill-current" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M50 65.626a3.125 3.125 0 1 1-6.25 0 3.125 3.125 0 0 1 6.25 0Zm41.513-10.068s.006 0 .006.002a1.56 1.56 0 0 0-1.04 1.472c0 .415.167.787.431 1.067l3.572 3.572a8.228 8.228 0 0 1 0 11.636 8.213 8.213 0 0 1-2.469 1.685l-5.545 2.334a2.337 2.337 0 0 0-1.112 2.898c.043.114.075.203.096.27l1.773 4.282c2.313 5.58-.337 11.978-5.919 14.29-5.58 2.311-11.978-.34-14.29-5.92l-1.663-4.016c-.003-.003-.01.01-.012.005a2.746 2.746 0 0 0-2.512-1.634c-.367 0-.714.084-1.036.215l-5.27 2.22h-.01a9.253 9.253 0 0 1-12.763-8.563v-3.247a3.125 3.125 0 0 1 6.25 0v3.125c0 1.726 1.399 3.125 3.125 3.125.383 0 .746-.083 1.085-.208l5.204-2.188c.054-.021.096-.024.153-.049 4.492-1.875 9.654.245 11.53 4.737 0 .003.002.006.004.009l1.689 4.077a4.687 4.687 0 1 0 8.66-3.587l-1.829-4.418c-1.787-4.284.165-9.188 4.37-11.105l5.663-2.383c.004 0-.005-.003 0-.003a1.525 1.525 0 0 0 .54-2.5l-7.35-7.35a5.506 5.506 0 0 1-1.595-3.877A5.53 5.53 0 0 1 86.779 50h.721a6.25 6.25 0 0 0 6.25-6.25V12.5a6.25 6.25 0 0 0-6.25-6.25h-75a6.25 6.25 0 0 0-6.25 6.25v31.25A6.25 6.25 0 0 0 12.5 50h28.125c1.726 0 3.125-1.4 3.125-3.125V32.263a8.826 8.826 0 0 1 15.079-6.228c.029.028.027.012.049.03l16.665 16.666a3.31 3.31 0 0 1 1.02 2.386 3.32 3.32 0 0 1-3.32 3.32 3.309 3.309 0 0 1-2.387-1.019L53.789 30.353c-.008-.006 0 .017-.012.003A2.197 2.197 0 0 0 50 31.885v16.552a7.812 7.812 0 0 1-7.813 7.814H12.5C5.597 56.25 0 50.653 0 43.75V12.5C0 5.597 5.597 0 12.5 0h75C94.403 0 100 5.597 100 12.5v31.25c0 5.495-3.558 10.134-8.487 11.808Z"
                            fill-rule="nonzero"></path>
                    </svg>
                </div>
            </div>
            <div
                class="divide-gray-750 border-gray-750 mt-3 flex h-8 h-full w-full items-center justify-center divide-x border-t text-xs">
                <span class="flex h-full w-1/2 items-center justify-center pr-3"><span>856</span>px</span>
                <span class="flex h-full w-1/2 items-center justify-center font-bold">md</span>
            </div>
        </div>
    </div>

    <div class="relative h-full w-auto overflow-scroll" style="display: none;">
        <div class="p-5 pb-12">
            <div>
                <h2 class="text-gray-400 mb-3 uppercase text-xs font-medium">Current Version</h2>
                <div
                    class="rounded flex items-center justify-center mt-3 group h-12 overflow-hidden cursor-pointer bg-gray-700">
                    <svg class="animate-spin h-4 w-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                            stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor"
                            d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
                        </path>
                    </svg>
                </div>
                <h2 class="text-gray-400 mb-3 mt-7 uppercase text-xs font-medium">Previous Versions</h2>

                <div class="text-yellow-900 rounded-t bg-yellow-500 text-[0.6rem] text-gray-400 p-2 px-3 relative">
                    <span
                        class="absolute right-0 top-0 -mr-3 -mt-3 bg-gray-800 rounded-full h-6 w-6 flex items-center justify-center">
                        <svg class="w-4 h-4 text-yellow-400" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <g fill="none" class="signal-icon-wrapper" stroke="none">
                                <path d="M8.543 17.25h6.915" stroke="currentColor" stroke-width="1.5"
                                    stroke-linecap="round" stroke-linejoin="round"></path>
                                <path
                                    d="M15.25 18.332c0-1.134.678-2.134 1.53-2.882a7.25 7.25 0 1 0-9.561 0c.853.748 1.53 1.747 1.53 2.882v.918a2 2 0 0 0 2 2h2.502a2 2 0 0 0 2-2v-.918z"
                                    stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
                                    stroke-linejoin="round"></path>
                            </g>
                        </svg>
                    </span>
                    <span>Click a previous version to preview it. Click the restore button and restore it.</span>
                </div>

                <div class="text-gray-400 rounded-b bg-gray-900 mb-3 text-[0.6rem] p-2 px-3 relative">
                    <span>Created every five minutes, on save.</span>
                </div>

                <div
                    class="rounded flex items-center justify-center group h-12 overflow-hidden cursor-pointer bg-gray-750">
                    <svg class="animate-spin h-4 w-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                            stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor"
                            d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
                        </path>
                    </svg>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- End Right Sidebar -->
